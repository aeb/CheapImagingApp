...
def read_array(array_file_name,existing_station_list=None) :
    ...
    sefd = np.loadtxt(array_file_name,usecols=[4])
    
    statdict = {}
    for j in range(len(stations)) :
        if (stations[j] in existing_station_list) :
            statdict[stations[j]] = {'on':True,'loc':locs[j],'sefd':sefd[j],'name':stations[j],'exists':True,'diameter':None}
        else :
            statdict[stations[j]] = {'on':True,'loc':locs[j],'sefd':sefd[j],'name':stations[j],'exists':False,'diameter':6}
